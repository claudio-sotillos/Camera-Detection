...
class DetectionTask(pl.LightningModule, TFLogger):
    ...
    def training_epoch_end(self, outputs: List[Any]) -> None:
        avg_loss = torch.stack(self.train_loss_list).mean()
        self.log("train_loss", avg_loss, on_epoch=True, prog_bar=True, logger=True ) # prog_bar=True, logger=True
    ...
    def test_step(self, batch, batch_nb):
        preds = self.model.infer(batch)
        # changed from 0 to 0.5 this is the threshold that decides if the image is included or not
        conf_threshold = self.hparams.get("conf_threshold", 0.5)
            ...
            for i, (sample, pred) in enumerate(zip(batch, preds)):
                ...
                for j in range(len(scores_nms)):
                    ...
                    if self.hparams.get("deploy", False):
                        ...
                        save_path = f"../data/goodImages/{panoid[:2]}/{panoid}_{heading}_{j}.jpg"  # changed path
                        json_save_path = f"../data/goodImages/{panoid[:2]}/{panoid}_{heading}_{j}.json"  # changed path
                        ...
                        img_box.save(f"../data/{batch_nb}_{i}.jpg")  # changed path
    ...
    def train_dataloader(self):
        ...
        return dataset.detection_dataloader(
            shuffle=True,
            augmentations=[
                T.RandomBrightness(0.5, 1.3),  # Add your image augmentations
    ...
    def test_dataloader(self):
        # if self.hparams.get('deploy', False):
        #     dataset = get_dataset(self.hparams['dataset_name'])
        ...
        # This code is changed since the code above did not work
        # The code beneath works

        if self.hparams.get('deploy', False):
            dataset = get_dataset(split="test")  # changed this to test
            print(self.hparams['deploy_meta_path'])
            df = pd.read_csv(self.hparams['deploy_meta_path']).query("downloaded == True")
            # print(df.head(2))
            df["image_id"] = df['save_path']
            df["gsv_image_path"] = df['save_path']
            ...
            # test_split = self.hparams.get("test_split")
